...
Select *from Comment
	...
	Select *from Comment
	Select Post_ID,COUNT(Post_Like_ID) from Post_Like group by Post_ID
	
	Update Post set Total_Likes=5 where Post_ID IN (3)
...
Select Comment_Id ,count(Comment_Like_ID) from Comment_Likes group by Comment_ID

Update Comment set Total_Likes=6 where Comment_ID IN (2)
...
Select Reply_ID, count(Reply_like_id) from Reply_Likes Group By Reply_Id

Update Reply set Total_Like=5 where Reply_ID IN(1,6)
...
Select Comment_id,COUNT(Reply_id) from Reply group by Comment_ID

Update Comment set Total_reply=5 where Comment_ID  IN(12)